/* eslint-disable import/no-default-export */
/* eslint-disable @typescript-eslint/no-non-null-assertion */
import { FileAddOutlined } from '@ant-design/icons/lib/icons';
import { observer } from 'mobx-react';
import React from 'react';
import useColorBinding from '../../hooks/useColorBinding';
import useModel from '../../hooks/useModel';
import ZFrame from '../../models/interfaces/Frame';
import { NullableReactElement } from '../../shared/type-definition/ZTypes';
import {
  BorderStyleDefaultDataAttributes,
  prepareBorderStyles,
} from '../side-drawer-tabs/right-drawer/config-row/BorderStyleConfigRow';
import { MRefProp } from './PropTypes';

export const ZFilePickerDefaultDataAttributes = {
  ...BorderStyleDefaultDataAttributes,
};

export type FilePickerAttributes = typeof ZFilePickerDefaultDataAttributes;

export const ZFilePickerDefaultFrame: ZFrame = {
  size: { width: 265, height: 150 },
  position: { x: 0, y: 0 },
};

export default observer(function ZFilePicker(props: MRefProp): NullableReactElement {
  const cb = useColorBinding();
  const model = useModel(props.mRef);
  if (!model) return null;

  const configuredStyle = prepareBorderStyles(model.dataAttributes as FilePickerAttributes, cb);
  return (
    <div style={{ ...styles.container, ...configuredStyle }}>
      <FileAddOutlined style={styles.icon} />
    </div>
  );
});

const styles: Record<string, React.CSSProperties> = {
  container: {
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    background: '#fff',
    width: '100%',
    height: '100%',
    overflow: 'hidden',
    backgroundColor: '#ddd',
  },
  icon: {
    fontSize: '32px',
    color: '#ffa522',
  },
};
