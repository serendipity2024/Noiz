...
import { VideoCameraAddOutlined } from '@ant-design/icons/lib/icons';
...
import { VideoSourceDefaultDataAttributes } from '../side-drawer-tabs/right-drawer/config-row/VideoSourceConfigRow';
import { MRefProp } from './PropTypes';
import { EventBinding } from '../../shared/type-definition/EventBinding';

export const ZVideoPickerDefaultDataAttributes = {
...
export type VideoPickerAttributes = typeof ZVideoPickerDefaultDataAttributes;

export const ZVideoPickerDefaultFrame: ZFrame = {
  size: { width: 265, height: 150 },
  position: { x: 0, y: 0 },
};

export default observer(function ZVideoPicker(props: MRefProp): NullableReactElement {
  const cb = useColorBinding();
  const model = useModel(props.mRef);
  if (!model) return null;

  const configuredStyle = prepareBorderStyles(model.dataAttributes as VideoPickerAttributes, cb);
  return (
    <div style={{ ...styles.container, ...configuredStyle }}>
      <VideoCameraAddOutlined style={styles.icon} />
    </div>
  );
});
