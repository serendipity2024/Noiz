...
const ListActions = (props: any) => {
  ...
  const preCommitCallback = async (action: 'create' | 'overwrite', values: any[]) => {
    const rows = await Promise.all(values.map(async value => {
      ...
      delete value['image'];
      if (value['image_id'] && isString(value['image_id']) && !isId(value['image_id'])) {
        const media = await processMedia(value['image_id'], MediaType.IMAGE) as {id: string, downloadUrl: string};
        value['image_id'] = media.id;
      }
      return value;
    }))
  ...
  const validateRow = async (value: any) => {
      ...
      if (value['image_id'] && isString(value['image_id']) && !isId(value['image_id'])) {
        const result = await validateMedia(value['image_id']);
        if (!result) throw Error('validateMediaError');
      }
  }
  ...
  const dynamicTyping = (columnName: string) => {
    if (_.includes(["status","category","title"], columnName))
      return false;
    return true;
  }
...
<TextInput source="category">
        ...
        <NumberInput source="images_image_storage_≤">
...
<TextInput source="title">
      ...
      export const BannerList = (props:any) => (
        <List empty={false} {...props} hasEdit={true} hasList={true} 
          ...
          bulkActionButtons={< ListBulkActions />} filters={<BannerFilter />}
        >
          <Datagrid expand={<BannerShow />} >
...
<ReferenceField label="images" source="images_image_storage" reference="image_storage" link="show">
<ChipField source="id">
...
<TextField source="title">
      ...
      export const BannerEdit = (props:any) => (
...
<RichTextInput component="pre" multiline="true" source="category">

</RichTextInput>
<DateInput source="banner_date">
...
<ReferenceInput label="images" source="images_image_storage" reference="image_storage" allowEmpty={true}>
...
<RichTextInput component="pre" multiline="true" source="title">

</RichTextInput>
      <ReferenceArrayInput label="imagee" source="imagee" reference="image_storage">
<SelectArrayInput allowEmpty={true} optionText="id">
      ...
      export const BannerCreate = (props:any) => (
        <Create {...props}>
          <SimpleForm>
            <NumberInput source="id" disabled={true}>
...
<RichTextInput component="pre" multiline="true" source="title">
      ...
      export const BannerShow = (props:any) => (
        <Show {...props}>
          <SimpleShowLayout>
...
<TextField source="title">
      ...
      <ReferenceManyField label="imagee" target="bannerr_banner" reference="image_storage">
