...
const ListActions = (props: any) => {
  ...
  const preCommitCallback = async (action: 'create' | 'overwrite', values: any[]) => {
    const rows = await Promise.all(values.map(async value => {
      ...
      if (value['profile_image_id'] && isString(value['profile_image_id']) && !isId(value['profile_image_id'])) {
        const media = await processMedia(value['profile_image_id'], MediaType.IMAGE) as {id: string, downloadUrl: string};
        value['profile_image_id'] = media.id;
      }
      return value;
    }))
  ...
  const validateRow = async (value: any) => {
      ...
      if (value['profile_image_id'] && isString(value['profile_image_id']) && !isId(value['profile_image_id'])) {
        const result = await validateMedia(value['profile_image_id']);
  ...
  const dynamicTyping = (columnName: string) => {
    if (_.includes(["username","password"], columnName))
      return false;
    return true;
  }
...
<TextInput source="password">
      ...
      export const AccountList = (props:any) => (
        <List empty={false} {...props} hasEdit={true} hasList={true} 
          ...
          bulkActionButtons={< ListBulkActions />} filters={<AccountFilter />}
        >
          <Datagrid expand={<AccountShow />} >
...
<FunctionField render={(record: any) => record.oauth2_user_info_map ?
              <pre>{JSON.stringify(record.oauth2_user_info_map, null, 2)}</pre> : <></>} source="oauth2_user_info_map">
...
<ImageField source="profile_image.url">
...
<RichTextInput component="pre" multiline="true" source="username">

</RichTextInput>
<RichTextInput component="pre" multiline="true" source="password">

</RichTextInput>
<TextInput multiline="true" source="oauth2_user_info_map">
...
<ImageInput accept="image/*" source="profile_image">
